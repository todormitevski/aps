...
public class Heroes {
    ...
    public static void startHeroesGame(SLL<Card> firstFriendCards, SLL<Card> secondFriendCards) {
        SLLNode<Card> strongest=firstFriendCards.getFirst();
        int maxDmg=firstFriendCards.getFirst().element.damage();
        Card strongestCard=new Card(0,0,0); //container for highest damage card
        while(strongest!=null){
            if(strongest.element.damage()>maxDmg){
                maxDmg=strongest.element.damage();
                strongestCard=strongest.element;
            }
            strongest=strongest.succ;
        }
        strongest=firstFriendCards.find(strongestCard);
        firstFriendCards.delete(strongest);

        SLLNode<Card> playerTwoNewHand=secondFriendCards.getFirst();
        int br=-3;
        while(playerTwoNewHand!=null){
            if(br==-1)
                secondFriendCards.insertAfter(strongestCard,playerTwoNewHand);
            br++;
            playerTwoNewHand=playerTwoNewHand.succ;
        }
    }
